...
公开仓库：远端为 `git@github.com:myfines/fluffy-waddle.git`，当前 `master` 已推送到 commit `f7bba59`。由于本机 SSH 公钥认证失败，本次使用同一 GitHub 地址的一次性 HTTPS push 完成上传，origin 配置仍保持用户提供的 SSH 地址。仓库不包含未知许可的710轮廓、派生轮廓、exe、release 或 Godot缓存；干净 clone 使用许可明确的 `data/demo_regions.json` 启动。
