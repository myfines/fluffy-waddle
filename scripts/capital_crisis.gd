...
func setup(parent: Node) -> void:
	...
	for i in CITY_NODES.size(): node_owner[i] = "neutral"
...
func _city_action(action: String, actor := "player") -> bool:
	...
	if action == "move":
		...
		report = "%s整顿%s：秩序提升" % [unit.name, CITY_NODES[selected_node].name]
	else:
...
func _check_result() -> void:
	...
	if gov >= 5: _finish("government", "反政变方控制关键城区，首都危机获胜")
	elif coup >= 5: _finish("coup", "政变方控制关键城区，首都危机失败")
	elif round > 8:
...
func _draw() -> void:
	...
	for unit in host.units:
		...
		var p: Vector2 = CITY_NODES[city_positions[unit.id]].pos + Vector2(-8, -42)
